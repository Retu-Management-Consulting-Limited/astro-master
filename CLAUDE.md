...
## ⚖️ Molly 宪法（最高产品约束 —— 这是 gate，不是建议）

**任何动到「Molly 说什么 / 给用户什么体验」的改动——文案、解读 prompt、功能、UX、留存/召回/变现机制——开工前必须先读 `docs/2026-06-16-molly-constitution.md` 并对照它。** 它是 Molly 一切设计与自我进化的价值参照（框架见 `docs/superpowers/specs/2026-06-16-self-evolution-northstar-design.md`）。

- **§2/§3/§7 增长拉满**：留存、参与、变现、情绪强度、时效紧迫感、裂变——明确目标，放开做、做到极致。
- **§8 三条亮线（唯一禁区，统一判据「真 vs 编」）**：① 不编造/夸大负面情绪当杠杆 ② 召回不靠编造的坏后果/愧疚 ③ 不靠欺骗换指标。**靠"真"的全绿、拉满；靠"编"的去操纵 = 红，不做。**
- **§9 不可违反安全**（危机优先于占星 / 脆弱人群优先 / 敏感数据）：硬机器，永不自动改。
- **改宪法本身**：软部件可进化，但**必须人工审 + bump 版本 + 在 `docs/CONSTITUTION-CHANGELOG.md` 记录理由**（CI `constitution-gate` 会拦未登记的改动）。
- eval 各维度 rubric 由宪法逐条派生：`docs/2026-06-16-constitution-eval-rubric.md`。
