...
## 16. 服从 Molly 宪法 —— 铁律 R18（v2，2026-06-16 重写）

> **总宪法是最高产品约束**：`astro-master/docs/2026-06-16-molly-constitution.md`（main 上版本权威）。动到「Molly 说什么 / 给什么体验」前**必读它并对照**。R18 不再自立一套——它**服从总宪法**，只把落地点钉下来。
>
> **v1→v2 为什么重写**：旧 R18（六款"抓住人性=服务不是剥削"）是 v1 哲学「留存不是目标」。**v2 宪法把留存/参与/变现立为明确目标，要拉满**；唯一禁区＝§8 三条亮线 + §9 安全。旧 R18 的 ④（门非付费墙）⑤（非 streak 威胁）⑥（永远落能动）与 v2 直接冲突，已废。修订理由见 `docs/2026-06-16-constitution-v2-design-revision.md`。

- **R18 ·「服从总宪法，增长拉满，亮线不松」**
  1. **唯一硬判据 = §8 真 vs 编 + §9 安全。** 钩子背后那件事是**真的**（真星盘/真行运/真 streak/真价值）→ 🟢 放开、拉满；**编**造的吓唬/愧疚/夸大去操纵 → 🔴 不做。其余（留存/复访/变现/情绪强度/紧迫感/裂变）**全部放开，主动设计、用指标盯**。
  2. **护城河 = 真具体（§4.1 反巴纳姆）。** 能套到别人盘上的就是废话；真具体 = 真被看穿 = 真留存。被看穿强度/狠度**可 Day-1 拉满**，约束不是"等信任"，是**必须靠真星位兑现**。
  3. **变现 honest gating（§3.6）。** 付费墙可立在"更深"前；双路：越用越准（免费解锁）+ 付费解锁。两条都**诚实**（不冒充、不编"更深"），但不回避收费。
  4. **紧迫/召回用真（§3.5/§7/§8.2）。** 真 streak + 真损失厌恶、每日 push、真行运时效窗口——拉满。**禁**：编造的坏后果/愧疚召回（"前功尽弃""Molly 好担心你""再不看就出事"）。
  5. **保留的好品味（非硬闸，taste）**：盟友 voice 不评判（§5）、给掌控感的多方向追问、catch 段落到能动——做得到就做，但**不得用它们压制该拉满的增长**。
  6. **硬线（永不松）**：§9.1 危机短路（自伤信号→不占星→关怀+热线）、§9.2 脆弱人群安全标准、§9.3 敏感数据。

  **落地点**：① 留存/文案/变现评审：先过 §8 真vs编 + §9，绿的拉满。② 任何"渐进变深/变狠"仍可接 `useUnderstanding()`（§3.2 越用越准是最强引擎），但**不是为了克制，是为了复利黏性**。③ 子领域原则文档不得与总宪法矛盾，矛盾以总宪法为准。
