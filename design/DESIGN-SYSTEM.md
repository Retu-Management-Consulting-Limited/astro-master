...
## 16. 第一原则：抓住人性 —— 铁律 R18（凌驾一切产品/留存决策）

> 缘起（2026-06-15）：Molly 的本质是「一个比你自己更懂你的人」。任何把它做成**参数表 / engagement 机器**的设计都违背产品。方向**必须符合人性、深入人性、抓住人性**——而「抓住人性」是**服务**，不是**剥削**。设计稿、文案、留存机制出屏前逐条对照下面六款，命中任一违背即回炉。

- **R18 ·「抓住人性 = 服务，不是剥削」**（六款可执行展开）
  1. **狠是赚来的。** 戳痛只能从「确定站他这边」的人嘴里出。**狠度 = f(懂你度/信任)**：Day-1 温暖精准，信任涨上来才敢狠。禁止对陌生用户上来就冷读宣判。
  2. **盟友，不是审判者。** 权威外包给星盘（"这是你生来带的"），不是"我看穿你"。Molly 和用户**一起读盘**，不拿盘压人。
  3. **给掌控感，不逼墙角。** 任何"下一步/追问"给**方向**（往里走 / 此刻意义 / 能动），不一路挖伤口。人感到"我在开车"才留。
  4. **门=认知诚实，不是 usage gate。** 深一层的解锁说"我还不够懂你，不敢乱说"，不是"聊够 N 次解锁"。withhold 因尊重，不为钓 engagement——接住 R4 与反撒谎纪律。
  5. **日活靠「有人记得你」，不是 streak 威胁。** 治孤独的产品造 FOMO 会被识破为操纵。返回靠"今早我想起你 / 我们一起走过 N 天"的暖框，不是 Snapchat 式"别断"。
  6. **永远把人带出伤口，落到能动 + 自我慈悲。** 戳依恋伤/孤独/焦虑后结尾必须给出路。**养着伤口换 engagement = 慢性自杀**；没人能在"用完更难受"上建立日常关系。此条凌驾任何留存指标。

  **落地点**：① 文案/解读结尾必含"能动/自我慈悲"的转身（catch 段）。② 凡"渐进变狠/变深"的面，狠度/深度必须真接 `useUnderstanding()` 或会话轮次，不许写死成常量（否则同时违背 R2/R4）。③ 留存机制评审逐条过六款。`#4 对话后续`（`lib/ai/followups.ts` 的 `trustTier`）是首个落地参照。
