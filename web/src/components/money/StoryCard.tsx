...
export function StoryCard({
  ...
  const t = useTranslations("money.story");
  return (
    <div data-testid="money-story" className="rounded-2xl border border-[rgba(127,201,154,0.3)] bg-[linear-gradient(180deg,rgba(127,201,154,0.1),rgba(127,201,154,0.02))] p-4">
      <div className="mb-2 flex items-center justify-between">
        <span className="text-[10px] uppercase tracking-[0.16em] text-[#c9a861]">{t("label")}</span>
        <span className="text-[10px] tracking-[0.1em] text-[#7a8194]">
          {isDayOne ? t("pageDayOne") : t("pageToday", { page })}
        </span>
      </div>
      {prev && !isDayOne ? (
        <p className="mb-2 border-b border-white/10 pb-2 text-[12px] leading-relaxed text-[#7a8194]">{t("prev", { prev })}</p>
      ) : null}
