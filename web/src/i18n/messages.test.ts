...
// a missing-key error at build/runtime.

function keysOf(obj: Record<string, unknown>): string[] {
  const out: string[] = [];
  for (const [k, v] of Object.entries(obj)) {
    if (v && typeof v === "object" && !Array.isArray(v)) {
      for (const sub of keysOf(v as Record<string, unknown>)) out.push(`${k}.${sub}`);
    } else {
      out.push(k);
    }
  }
...
describe("i18n messages: Task 7 namespaces", () => {
  ...
  it("zh and ru have identical key structure (no missing translations)", () => {
    expect(keysOf(ru as Record<string, unknown>)).toEqual(
      keysOf(zh as Record<string, unknown>),
    );
  });
});
